...
public class SocJumpFunction {
    private static Logger logger = Logger.getLogger(SocJumpFunction.class);

    public static DataStream<Pace> Main(DataStream<CarState> dataStream) {

        DataStream<Pace> socJumpDataStream = dataStream
    ...
    static class SocJumpGoogleSortedProcessFunc extends ProcessWindowFunction<CarState, Pace, String, TimeWindow> {
                ...
                if (value != null) {
                    ...
                    if (valueTime - startMills > BizConstant.SOC_JUMP_TIME_OUT) {
                        ...
                        if (changedSoc >= BizConstant.SOC_JUMP_THRESHOLD) {
                            Pace jumpPoint = new Pace();
                            jumpPoint.setDataType("SocJump");
                            jumpPoint.setVin(key);
                            jumpPoint.setStartSoc(valueSoc);
                            jumpPoint.setStartTime(valueTime);
                            jumpPoint.setEndSoc(soc);
                            jumpPoint.setEndTime(currentTime);
